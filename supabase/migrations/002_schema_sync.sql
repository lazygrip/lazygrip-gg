...
-- ============================================================
-- IDEMPOTENCY REPAIR, 2026-09-17
-- ============================================================
-- Every `create table if not exists` above is a no-op on any database that
-- ran 001, because 001 already created profiles, sequences, ratings,
-- comments, saves and sequence_versions -- in a narrower shape. So on a
-- FRESH apply this file declared 41 columns on public.sequences and added
-- none of them, and then died at the first policy naming `status`.
--
-- These statements add exactly the columns declared above and nothing else;
-- the list is generated by diffing 001's CREATE TABLE bodies against this
-- file's. Every one is `if not exists`, so on a database where this
-- migration has already run they are all no-ops.
-- See scripts/schema-check/ and issue #78.

-- profiles: 1 column(s) declared above that 001 does not create
alter table public.profiles add column if not exists avatar_color text;

-- ratings: 1 column(s) declared above that 001 does not create
alter table public.ratings add column if not exists reason_tags text[];

-- sequence_versions: 10 column(s) declared above that 001 does not create
alter table public.sequence_versions add column if not exists grip_string text not null;
alter table public.sequence_versions add column if not exists version_number integer not null default 1;
alter table public.sequence_versions add column if not exists author_id uuid;
alter table public.sequence_versions add column if not exists hero_talent text;
alter table public.sequence_versions add column if not exists content_type text;
alter table public.sequence_versions add column if not exists step_function text;
alter table public.sequence_versions add column if not exists grip_version text;
alter table public.sequence_versions add column if not exists talent_string text;
alter table public.sequence_versions add column if not exists warcraftlogs_url text;
alter table public.sequence_versions add column if not exists performance_notes text;

-- sequences: 28 column(s) declared above that 001 does not create
alter table public.sequences add column if not exists class_id integer not null;
alter table public.sequences add column if not exists class_name text not null;
alter table public.sequences add column if not exists spec_id integer;
alter table public.sequences add column if not exists spec_name text;
alter table public.sequences add column if not exists content_type text not null;
alter table public.sequences add column if not exists hero_talent text;
alter table public.sequences add column if not exists patch_version text;
alter table public.sequences add column if not exists grip_version text;
alter table public.sequences add column if not exists step_function text;
alter table public.sequences add column if not exists step_count integer;
alter table public.sequences add column if not exists grip_string text;
alter table public.sequences add column if not exists raw_steps jsonb;
alter table public.sequences add column if not exists keybind_info jsonb;
alter table public.sequences add column if not exists talent_string text;
alter table public.sequences add column if not exists warcraftlogs_url text;
alter table public.sequences add column if not exists view_count integer default 0;
alter table public.sequences add column if not exists avg_score numeric;
alter table public.sequences add column if not exists rating_count integer not null default 0;
alter table public.sequences add column if not exists comment_count integer not null default 0;
alter table public.sequences add column if not exists current_version_id uuid;
alter table public.sequences add column if not exists current_version_label text;
alter table public.sequences add column if not exists set_id uuid;
alter table public.sequences add column if not exists collection_sequences jsonb;
alter table public.sequences add column if not exists original_author text;
alter table public.sequences add column if not exists attribution_acknowledged_at timestamptz;
alter table public.sequences add column if not exists discord_thread_id text;
alter table public.sequences add column if not exists last_discord_notified_at timestamptz;
alter table public.sequences add column if not exists status public.sequence_status not null default 'draft';
