...
/cast Ironfur`,
      ...
      <GuideSection title="Charged and empower spells: why they break automated sequencing">
        ...
        <p style={{ marginTop: 16 }}>As of v2.4.14, start with the setting built for exactly this problem before reaching for either workaround below. The other two approaches here predate that fix and exist specifically to route around the automated-cancel hitch by hand; the setting solves it directly instead.</p>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginTop: 12 }}>
          <div style={{ padding: '14px 16px', background: 'var(--bg-primary)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-md)' }}>
            <p style={{ fontSize: 'var(--text-base)', fontWeight: 600, color: 'var(--text-primary)', marginBottom: 6 }}>Works, and try this first: Hold While Channeling</p>
            <p style={{ fontSize: 'var(--text-base)', color: 'var(--text-secondary)', lineHeight: 1.6 }}>Covered in full on the <Link href="/guide/settings" style={{ color: 'var(--accent)', textDecoration: 'none', fontWeight: 500 }}>Settings</Link> page. This setting, and specifically its Hold + release at max option, holds a press landing mid-channel instead of letting it cancel the charge early, then releases at the point you asked it to. Through v2.4.13 it only appeared for characters with an actual empowered spell in their spellbook, in practice Evoker, even though the underlying mechanism has always worked on any channel. That was a UI bug, fixed in v2.4.14, so it now applies cleanly to Fire Breath and Upheaval without the guard-step or separate-key workarounds below.</p>
          </div>
          <div style={{ padding: '14px 16px', background: 'var(--bg-primary)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-md)' }}>
            <p style={{ fontSize: 'var(--text-base)', fontWeight: 600, color: 'var(--text-primary)', marginBottom: 6 }}>Also works, predates the setting above: a dedicated first-priority guard step</p>
...
/cast [channeling:Fire Breath] Fire Breath`}</CodeBlock>
          ...
          <div style={{ padding: '14px 16px', background: 'var(--bg-primary)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-md)' }}>
            <p style={{ fontSize: 'var(--text-base)', fontWeight: 600, color: 'var(--text-primary)', marginBottom: 6 }}>Also works, predates the setting above: fully separate sequences per charge spell, on separate keys</p>
        ...
        <p style={{ marginTop: 16 }}>None of this is Evoker specific in principle, it applies to any spell WoW treats as charged or empowered, but Evoker is currently the class where it comes up. If you are building a sequence around Aug Evoker's Ebon Might upkeep or a Devastation opener that leans on Fire Breath or Upheaval, start with Hold While Channeling and fall back to the guard-step or separate-key approach only if it does not behave the way you need for your spec, rather than assuming a normal cast-step pattern will behave the same way it does for every other ability in your kit.</p>
      </GuideSection>
