...
export default function FeaturesAndBehaviorPage() {
  return (
    <div style={{ maxWidth: 720 }}>
      ...
      <GuideSection title="Viewing sequences from other characters">
        <p>GRIP-EMS sequences are stored per character. That is not a setting anyone picked, it is how WoW loads addon saved data: only the file belonging to the character you are currently logged in as gets loaded, so an alt&apos;s sequences are not hidden, they are not in memory at all while you are playing a different character. A brand new character showing an empty sequence list, with the search box finding nothing, is that working as intended rather than a bug.</p>
        <p style={{ marginTop: 12 }}>There is a way to see across characters without switching. The sequence list carries an Other characters tick box right under Current class only, off by default. Turn it on and you get a read-only view of every other character&apos;s sequences, names, icons, and step counts, once each character has logged in at least once on a build that has this feature; each character publishes its own list on login, so a fresh alt only appears in that list after you have logged into it once. You cannot fire another character&apos;s sequence from that view, but the copy to character option on any sequence stages it to actually land on that character the next time you log into it there.</p>
      </GuideSection>
