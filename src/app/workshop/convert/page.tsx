  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '32px 24px' }}>
      <div style={{ marginBottom: 24, display: 'flex', alignItems: 'center', gap: 12 }}>
        ...
        <span style={{ fontSize: 'var(--text-sm)', color: 'var(--text-secondary)' }}>Convert to GRIP</span>
      </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div style={{ padding: '14px 16px', background: 'var(--bg-secondary)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-lg)' }}>
                ...
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                  {result.sequenceNames.map(name => (
                    <span key={name} style={{ fontSize: 'var(--text-xs)', padding: '2px 8px', background: 'var(--bg-tertiary)', borderRadius: 'var(--radius-sm)', color: 'var(--text-secondary)' }}>{name}</span>
                  ))}
                </div>
              </div>
              ...
              <div style={{ background: 'var(--bg-secondary)', border: '0.5px solid var(--border)', borderRadius: 'var(--radius-lg)', overflow: 'hidden' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px', borderBottom: '0.5px solid var(--border)' }}>
                  <span style={{ fontSize: 'var(--text-xs)', fontWeight: 500, color: 'var(--text-secondary)' }}>GRIP export string</span>
                  <button
                    onClick={copyExport}
                    style={{
              ...
              {result.warnings.length > 0 && (
                <div style={{ padding: '14px 16px', background: 'rgba(255,180,0,0.07)', border: '0.5px solid rgba(255,180,0,0.25)', borderRadius: 'var(--radius-lg)' }}>
                  <p style={{ fontSize: 'var(--text-xs)', fontWeight: 600, color: '#c8960c', marginBottom: 8 }}>Conversion notes</p>
                  {result.warnings.map((w, i) => (
                    <p key={i} style={{ fontSize: 'var(--text-xs)', color: 'var(--text-secondary)', lineHeight: 1.6 }}>• {w}</p>
                  ))}
                </div>
              )}
