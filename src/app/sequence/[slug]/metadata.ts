...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!sequence) {
    return {
      title: 'Sequence Not Found | LazyGrip.net',
  ...
  const authorPart = (sequence.author as any)?.username ? ` by ${(sequence.author as any).username}` : ''

  const title = `${sequence.title} | LazyGrip.net`
