...
class ImportDataStream {
  ...
  constructor(exportString: string | null) {
    this.values = [...String(exportString || "").trim()].map(char => BASE64_TO_VALUE[char]);
    if (!this.values.length || this.values.some(value => value === undefined)) {
...
function parseTalentImportHeader(talentString: string | null): { serializationVersion: number; specId: number } | null {
  try {
    const stream = new ImportDataStream(talentString);
    const headerBits = HEADER_VERSION_BITS + SPEC_ID_BITS + 128;

    if (stream.values.length * BITS_PER_CHAR < headerBits) {
      return null;
    }
    ...
    if (!serializationVersion || !specId) {
      return null;
    }
...
function extractTalentStringFromText(text: unknown): string | null {
  ...
  if (labeledMatch) {
    return normalizeTalentToken(labeledMatch[1]);
  }

  for (const line of cleaned.split(/\r?\n/)) {
    const token = normalizeTalentToken(line);
    if (isLikelyTalentString(token)) {
      return token;
    }
  }
...
function findTalentStringInComments(exportMeta: any, sequences: any[]): string | null {
  const sources = [
    exportMeta?.description,
    ...(sequences || []).map(sequence => sequence.description)
  ];
...
function normalizeTalentToken(value: unknown): string {
  return String(value || "")
    .trim()
    .replace(/^["']|["']$/g, "")
    .replace(/[.,;)\]}>]+$/g, "");
...
function isLikelyTalentString(value: string | null | undefined): boolean {
  if (!value || value.length < MIN_TALENT_LENGTH) {
    return false;
  }

  if (!TALENT_TOKEN_PATTERN.test(value)) {
    return false;
  }
...
function stripTalentFromText(text: unknown, talentString: unknown): string {
  const cleaned = String(text || "");
  const token = normalizeTalentToken(talentString);
  if (!cleaned.trim() || !token) {
    return cleaned;
  }
